...
const staggerContainer = {
  ...
  visible: {
    opacity: 1,
    transition: {
      staggerChildren: 0.15, // Delay between each member appearing
      delayChildren: 0.2
    }
  }
}
...
export default function TeamSection() {
  ...
  const teamsData = useMemo(() => {
    const yearData = teamData[activeYear]
    if (!yearData) return []
    ...
    // Faculty Advisors
    if (yearData.facultyAdvisors?.length) {
      addTeam('Faculty Advisors', yearData.facultyAdvisors[0], yearData.facultyAdvisors, 'gridOnly')
    }
    ...
    const departmentMap: { [key: string]: string } = {
      ...
      prMarketing: 'PR & Marketing', creative: 'Creative', socialMedia: 'Social Media',
      events: 'Events', logistics: 'Logistics',
    }

    for (const key in departmentMap) {
      if (yearData[key]?.head) {
        addTeam(departmentMap[key], yearData[key].head, yearData[key].team || [])
      }
    }
    
    // Student Advisors
    if (yearData.studentAdvisors?.length) {
      addTeam('Student Advisors', yearData.studentAdvisors[0], yearData.studentAdvisors, 'gridOnly')
    }
    ...
    <section id="team" className="relative min-h-screen bg-transparent py-24 px-6 md:px-12 overflow-hidden">
      <div className="max-w-7xl mx-auto mb-16 relative z-10">
        ...
        >
          ...
          <span className="text-sm font-medium tracking-[0.2em] text-neutral-400 uppercase">Behind the scenes</span>
        </motion.div>
      ...
      <div className="max-w-7xl mx-auto space-y-24">
        {teamsData.map((team) => (
          <div key={team.name} className="relative group">
            ...
            {/* CASE 1: Grid Only (Faculty, Core, Advisors) - CENTERED */}
            {team.variant === 'gridOnly' ? (
              ...
              // This creates visual harmony by aligning the vertical midpoint of the member photos with the vertical center of 
              // the head's photo, making the layout balanced and preventing the members from appearing misaligned or floating 
              // awkwardly at the top. This is especially important when there are fewer members, as it maintains symmetry and 
              // professional appearance across different team sizes.
              <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-12 lg:gap-16 items-center">
                ...
                >
                  <div className="relative w-full aspect-[4/5] rounded-3xl overflow-hidden border-2 border-neutral-900 shadow-2xl bg-white">
                    <Image
                      src={team.head.image ? `/${team.head.image}` : '/placeholder.svg'}
                      alt={team.head.name}
                      fill
                ...
                {/* DON'T REMOVE: justify-center is crucial - it centers member photos horizontally relative to the team head,
                    creating visual balance and alignment. The head (280px) is larger than members (220px max), 
                    so centering prevents members from awkwardly bunching to the left edge of the grid column. */}
                <motion.div 
                  variants={staggerContainer}
                  initial="hidden"
                  whileInView="visible"
    ...
    <div className="group/member relative w-full">
      <div className={`relative w-full aspect-[4/5] ${small ? 'rounded-2xl' : 'rounded-3xl'} overflow-hidden border border-neutral-200 shadow-sm bg-white transition-all duration-300 hover:shadow-xl hover:-translate-y-2`}>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-4 group-hover/member:translate-y-0 opacity-0 group-hover/member:opacity-100 transition-all duration-300">
          ...
          {/* Social Icons (Optional based on your types) */}
